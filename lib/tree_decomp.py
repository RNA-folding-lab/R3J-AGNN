...
class RNAJTNode:
    def __init__(self, hpn_label, nt_idx_assignment, **kwargs):
        ...
        # self.base_idx = None  # 新增属性
        # print(f"RNAJTNode initialized with base_idx = {self.base_idx}")

    # def set_base_idx(self, rna_seq):
    #     bases = []
    #     indices = []
    #     for segment in self.nt_idx_assignment:
    #         if isinstance(segment, list):
    #             bases.append(''.join([rna_seq[i] for i in segment]))
    #             indices.extend(segment)
    #         else:
    #             bases.append(rna_seq[segment])
    #             indices.append(segment)
    #     self.base_idx = (bases, [i+1 for i in indices])
...
class RNAJunctionTree:
    ...
    def __init__(self, rna_seq, rna_struct, **kwargs):
        ...
        if rna_struct is not None and type(rna_struct) is str:
            ...
            # self.rna_struct = rna_struct.replace('&', '')
            
            self.rna_struct = re.sub(r'[\[\]{}]', '.', rna_struct)
            ...
            #     node.set_base_idx(self.rna_seq)
    ...
    # # 碱基序号及打印
    # def get_node_bases_with_indices(self, node_idx):
    #     node = self.nodes[node_idx]
    #     bases = []
    #     indices = []
    #     for segment in node.nt_idx_assignment:
    #         if isinstance(segment, list):
    #             bases.append(''.join([self.rna_seq[i] for i in segment]))
    #             indices.extend(segment)
    #         else:
    #             bases.append(self.rna_seq[segment])
    #             indices.append(segment)
    #     return bases, indices
    
    # 打印数据
    def get_node_bases(self, node_idx):
        node = self.nodes[node_idx]
        bases = []
        for segment in node.nt_idx_assignment:
            if isinstance(segment, list):
...
def decompose(dotbracket_struct):
    # 预处理 dotbracket_struct，将 '[]' 替换为 '.'
    # dotbracket_struct = dotbracket_struct.replace('&', '.')
    # dotbracket_struct = re.sub(r'[\[\]{}]', '.', dotbracket_struct)
    # print("dotbracket_struct:", dotbracket_struct)
    # 首先移除所有 '&' 符号
    # dotbracket_struct = dotbracket_struct.replace('&', '')
    ...
    # ================= 修复补丁开始 =================
    # 查找所有被 forgi 遗漏的零长度 Hairpin
    # 逻辑：如果是 Stem，且只有一个邻居，且这个邻居不是 Dangling Start/End 或 Pseudo
    for i, node_id in enumerate(all_hpn_ids):
        if node_id.startswith('S'):
            # 获取当前 Stem 的邻居数量
            current_neighbors = hpn_neighbors[i]
            
            # 如果只有一个邻居（说明它是叶子），我们需要检查它是否缺 Hairpin
            if len(current_neighbors) == 1:
                neighbor_idx = current_neighbors[0]
                neighbor_id = all_hpn_ids[neighbor_idx]
                
                # 如果唯一的邻居是 Multiloop (M) 或 Internal loop (I) 或 Pseudo (P)
                # 且这个 Stem 不是连接到 RNA 5'/3' 末端的 Stem（通常 P 连接的是末端 Stem）
                # 注意：这里简化逻辑，假设所有悬空的内部 Stem 都应该有个 Hairpin
                
                # 获取 Stem 的索引范围
                stem_indices = hypernodes[node_id] 
                # stem_indices[0] 是 5' 端片段 (如 [34...39])
                # stem_indices[1] 是 3' 端片段 (如 [40...45])
                
                s_5_end = stem_indices[0][-1] # 39
                s_3_start = stem_indices[1][0] # 40
                
                # 检查是否是零长度环 (索引相邻)
                if s_5_end + 1 == s_3_start:
                    # 确实是一个零长度环，手动添加 Hairpin 节点
                    new_h_id = f"H_fixed_{i}"
                    # 定义 Hairpin 覆盖的碱基：[39, 40] (即 Stem 的闭合对)
                    # 对应原始逻辑 range(start-2, end+1) -> range(39, 41)
                    hypernodes[new_h_id] = [s_5_end, s_3_start]
                    
                    # 更新 ID 列表
                    all_hpn_ids = np.append(all_hpn_ids, new_h_id)
                    
                    # 更新邻居关系
                    # 1. 为新 Hairpin 添加邻居 (指向 Stem)
                    hpn_neighbors.append([i])
                    # 2. 为 Stem 添加邻居 (指向新 Hairpin)
                    hpn_neighbors[i].append(len(all_hpn_ids) - 1)
                    
                    print(f"DEBUG: Fixed missing hairpin for stem {node_id} at indices {s_5_end}-{s_3_start}")
    ...
    hpn_nodes_assignment = np.array(list(hypernodes.values()), dtype=object)[breadth_first_order]
...
if __name__ == "__main__":
    ...
    rna_name = 'PZ19_solution_0'
    rna_seq = 'GCAGGGCAAGGCCCAGUCCCGUGCAAGCCGGGACCGCCCCGGGGCGCGGCGCUCAUUCCUGC'
    rna_struct = '(((((...((((([.((((((.....].))))))(((((()))))).))).))....)))))'

    # rna_name = '1Y26_X'
    # rna_seq = 'CGCUUCAUAUAAUCCUAAUGAUAUGGUUUGGGAGUUUCUACCAAGAGCCUUAAACUCUUGAUUAUGAAGUG'
    # rna_struct = '((((((((((..((((((......[[.))))))[.....)]((((((]].....))))))..)))))))))'
    # 移除序列和结构中的&
    processed_seq = rna_seq.replace('&', '')
    processed_struct = rna_struct.replace('&', '')

    tree = RNAJunctionTree(processed_seq, processed_struct)
    stack = []
    dfs(stack, tree.nodes[1], 0)

    # 节点数据
    for tuple in stack:
        node1, node2, direction = tuple
        bases1 = tree.get_node_bases(node1.idx)
        bases2 = tree.get_node_bases(node2.idx)
        print(f"({node1.idx}, {node2.idx}, {direction})")
        print(f"Node {node1.idx} ({node1.hpn_label}): {bases1}")
        print(f"Node {node2.idx} ({node2.hpn_label}): {bases2}")
        print()
    ...
    edge_index = np.array(list(zip(*edges)))
    back_edge_index = np.array(list(zip(*back_edges)))

    print("Edge Index:")
    print(edge_index)
    print("Backward edge index:")
    print(back_edge_index)
